...
function openFilePicker() { // IDEA: look for Path not Name!! //Kinda Works
...
function saveDB() {// needs testing when loading is finished
  ...
  for (let i = 0; i < title.length; i++) {
    db.remove({path: pfad[i].innerText}, function(err, numRemoved) {
  ...
  db.insert(items, function(err, doc) {console.log(items);console.log('Saved!');})
}
